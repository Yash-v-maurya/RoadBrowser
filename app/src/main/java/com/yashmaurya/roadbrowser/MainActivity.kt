...
import com.yashmaurya.roadbrowser.AppConstants.FREE_DROID_WARN_SOLUTIONS_URL
import com.yashmaurya.roadbrowser.AppConstants.FREE_DROID_WARN_VERSION_KEY
import com.yashmaurya.roadbrowser.AppConstants.GITHUB_REPO_URL
import com.yashmaurya.roadbrowser.AppConstants.KEEP_ANDROID_OPEN_URL
import com.yashmaurya.roadbrowser.AppConstants.REQUEST_CODE_POST_NOTIFICATIONS
import com.yashmaurya.roadbrowser.AppConstants.REQUEST_CODE_RECORD_AUDIO
...
import com.yashmaurya.roadbrowser.navigation.NavigationManager
...
import com.yashmaurya.roadbrowser.ui.MainActivitySetup
import com.yashmaurya.roadbrowser.ui.OverlayManager
import com.yashmaurya.roadbrowser.ui.ThemeManager
...
import com.yashmaurya.roadbrowser.web.updateUserAgentProfile
import org.woheller69.freeDroidWarn.R as FreeDroidWarnR

/**
 * MainActivity serves as the central hub for the RoadBrowser application.
...
class MainActivity : AppCompatActivity() {
    ...
    var latestReleaseUrl: String = "https://github.com/Yash-v-maurya/RoadBrowser/releases"
        private set
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        com.yashmaurya.roadbrowser.media.MediaPlaybackService.actionHandler =
            com.yashmaurya.roadbrowser.media.MediaPlaybackService.MediaActionHandler { action ->
                runOnUiThread {
                    webView?.let { com.yashmaurya.roadbrowser.media.MediaSessionBridge.dispatch(it, action) }
                }
            }
    }
    ...
    override fun onResume() {
        ...
        uiManager.applyToolbarLayout()
    }

    override fun onPause() {
        // Fullscreen is deliberately kept across pauses: on a head unit the app loses focus
    ...
    private fun buildBrowserCallbacks(tab: BrowserTab): BrowserCallbacks {
        return BrowserCallbacks(
            ...
            onOpenInNewTab = { url ->
                // The web layer may call this off the UI thread and needs the answer straight
                // away, so the decision is made synchronously and only the tab creation is
                // marshalled onto the UI thread. Returning false makes the caller fall back to
                // loading the URL in the current WebView.
                val target = sanitizeNewTabUrl(url)
                val canOpen = target != null &&
                    BrowserPreferences.isOpenPopupsInNewTabEnabled(this) &&
                    tabManager.browserTabs.size < BrowserPreferences.MAX_OPEN_TABS
                if (canOpen) {
                    runOnUiThread {
                        tabManager.createBrowserTab(initialUrl = target, activate = true)
                    }
                }
                canOpen
            }
    ...
    /**
     * Popup targets come from the page, so they are validated the same way JS-provided URLs are
     * (see [BrowserUIManager.sanitizeJsExternalUrl]): only http(s) is accepted and anything
     * without a scheme goes through the shared address formatter. Returns null when the URL must
     * not be opened in a new tab.
     */
    private fun sanitizeNewTabUrl(rawUrl: String): String? {
        ...
        if (scheme == null) {
            return BrowserPreferences.formatNavigableUrl(candidate, this)
        }
    ...
    private fun updateNavigationButtons() {
        ...
        uiManager.updateToolbarState(canGoBack = binding.buttonBack.isEnabled, canReload = can)
    }
    ...
    private fun createStartPageCallbacks() = object : StartPageManager.StartPageCallbacks {
        ...
        override fun updateToolbarVisibility() {
            this@MainActivity.updateToolbarVisibility()
        }
    ...
    private fun createTabCallbacks() = object : TabManager.TabCallbacks {
        ...
        override fun onMediaStateChanged(tabId: Long, playing: Boolean, title: String, artist: String) {
        ...
        override fun applyToolbarLayout() {
            uiManager.applyToolbarLayout()
        }
